...
class IndexView(LoginRequiredMixin, NavTopicsMixin, generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        return super().get_context_data(**kwargs) | {
            ...
            'latest_newspapers': (
                Newspaper.objects.prefetch_related('topics', 'publishers')[:5]
            ),
        }
...
class TopicCreateView(
    LoginRequiredMixin, NavTopicsMixin, SuperuserRequiredMixin, generic.CreateView
):
    model = Topic
...
class TopicUpdateView(
    LoginRequiredMixin, NavTopicsMixin, SuperuserRequiredMixin, generic.UpdateView
):
    model = Topic
    form_class = TopicForm
    success_url = '/topics/'
...
class TopicDeleteView(
    LoginRequiredMixin, NavTopicsMixin, SuperuserRequiredMixin, generic.DeleteView
):
    model = Topic
    success_url = '/topics/'
...
class RedactorListView(NavTopicsMixin, SearchListView):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .annotate(newspaper_count=Count('newspapers', distinct=True))
        )
...
class RedactorUpdateView(
    LoginRequiredMixin, NavTopicsMixin, SelfOrSuperuserRequiredMixin, generic.UpdateView
):
    model = get_user_model()
